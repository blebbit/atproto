...
export default function (server: Server, ctx: AppContext) {
  server.com.atproto.space.createSpace({
    ...
    handler: async ({ auth, input }) => {
      ...
      const newSpaceType = record.bubble ? "bubble" : "space"
      const newSpaceSid = `${newSpaceType}:${repoDidSid}/${rkeySid}`
      const newRecordSid = `record:${repoDidSid}/${rkeySid}/${atproto2spicedb(LEX.ids.ComAtprotoSpaceSpace)}/self`
      ...
      await dualWriteTransaction({
        ...
        authzOps: async (spicedbClient) => {
          ...
          const r1 = await touchRelationship(spicedbClient, newSpaceSid, relation, parentSid)
          const r2 = await touchRelationship(spicedbClient, newRecordSid, relation, newSpaceSid)
