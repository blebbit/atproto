...
export default function (server: Server, ctx: AppContext) {
  server.com.atproto.space.createRelation({
    ...
    handler: async ({ auth, input }) => {
      ...
      let newObjectSid: string;
      let newSubjectSid: string;
      ...
      try {
        ...
        const valid = LEX.validate(record, LEX.ids.ComAtprotoSpaceRelation, "main")
        console.log("Reln Valid?", valid)
        ...
        // TODO: validate more record contents
        // repo DID & space match the resourceUri in the record
        ...
        // TODO, this might not be needed here, or only when the resource/object is a space probably
        //   and generally could benefit from caching in all handlers
        // TODO, we should make sure whatever it is, that it does exist in the first place
        // probably the same for the subject?
        // TODO, this db lookup code should be moved above the check permissions comment above
        ...
        // TODO, clean up naming (re: object vs resource, align with spicedb?)
        // TODO, make a helper that turns an at-uri into a spicedb id, reverse too for lookups
        const objectType = record.resourceType as string;
        const objectParts = (record.resource as string).split("/");
        const objectSpiceParts = objectParts.map(s => atproto2spicedb(s))
        const objectId = objectSpiceParts.join("/")
        const permission = `${objectType}_iam_creator`
        const subjectType = record.subjectType as string;
        const subjectParts = (record.subject as string).split("/");
        const subjectSpiceParts = subjectParts.map(s => atproto2spicedb(s))
        const subjectId = subjectSpiceParts.join("/")

        newObjectSid = `${objectType}:${objectId}`
        newSubjectSid = `${subjectType}:${subjectId}`

        // TODO, caveats for records, but probably only

        // here we are determining if the requesting account
        //   has *_iam_create over the resource/object
      ...
      // main record(s)
      const relationRecord = await prepareCreate({
        reqDid,
        repoDid,
        space: reqSpace,
      ...
      await dualWriteTransaction({
        ...
        authzOps: async (spicedbClient) => {
          ...
          const r = await touchRelationship(spicedbClient, newObjectSid, relation, newSubjectSid)
      ...
      // success!
      const body = {
        cid: relationRecord.cid.toString(),
        status: "ok",
        uri: relationRecord.uri.toString(),
        validationStatus: relationRecord.validationStatus,
        zookie: newZookie,
      };
