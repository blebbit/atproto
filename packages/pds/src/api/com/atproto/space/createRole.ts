  ...
  server.com.atproto.space.createRole({
    ...
    handler: async ({ auth, input }) => {
      ...
      const mainRecord = await prepareCreate({
        ...
        repoDid,
        space: reqSpace,
        collection: LEX.ids.ComAtprotoSpaceRole,
        rkey,
        record,
        validate,
      })
      ...
      const newResourceSid = `role:${repoDidSid}/${currSpaceSid}/${atproto2spicedb(LEX.ids.ComAtprotoSpaceGroup)}/${rkeySid}`
      ...
      const parentRelation = await prepareCreate({
        ...
        rkey: ctx.genCuid['16'](),
        record: {
          // TODO, these need to match the actual record format
          // missing types and should not be SIDs
          resourceType: "role",
          resource: newResourceSid,
          relation,
          subjectType: currSpaceType,
          subject: `${repoDid}/${reqSpace}`,
        },
      })
