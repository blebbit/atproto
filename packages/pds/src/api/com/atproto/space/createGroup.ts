    ...
    handler: async ({ input, auth }) => {
      ...
      try {
        ...
        const preflight = await preflightChecks({
          ...
          repo,
          space,
          collection: LEX.ids.ComAtprotoSpaceCreateGroup,
          rkey,
          record,
      ...
      const subject = `space:${aturi2spicedb(groupRecord.parentUri)}`
      const permission = 'parent'
      const resource = `space:${aturi2spicedb(groupRecord.uri)}`
