...
export class SpaceTransactor extends SpaceReader {
  ...
  async createSpace(
    ...
    // need to construct AtUri
    const uri = AtUri.make(repo, LEX.ids.ComAtprotoSpaceSpace, rkey, space)

    log.debug({ uri }, 'creating space')
    const row = {
      uri: uri.toString(),
      space,
      did,
    ...
    if (!uri.hostname.startsWith('did:')) {
      throw new Error('Expected AtURI to contain DID')
    } else if (row.space.length < 1) {
      throw new Error('Expected AtURI to contain a space')
    } else if (row.collection.length < 1) {
      throw new Error('Expected AtURI to contain a collection')
    } else if (row.rkey.length < 1) {
      throw new Error('Expected AtURI to contain a record key')
    }

    // Track current version of space
    await this.db.db
      .insertInto('space')
      .values(row)
      .onConflict((oc) =>
