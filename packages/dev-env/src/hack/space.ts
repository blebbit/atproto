...
const funcs = {
  describeRepo: {
    fn: async (agent: AtpAgent, { repo }) => {
      const r = await agent.com.atproto.space.describeRepo({
        repo,
      })
      return r.data
    },
    args: ['repo'],
    help: 'Describe a repo.',
  },
  createSpace: {
  ...
  putSpace: {
    fn: async (agent: AtpAgent, { repo, space, record }) => {
  ...
  createGroup: {
    fn: async (agent: AtpAgent, { repo, space, record }) => {
      const r = await agent.com.atproto.space.createGroup({
        repo,
        space,
        record: JSON.parse(record),
      })
  ...
  createRecord: {
    fn: async (agent: AtpAgent, { repo, space, collection, record, rkey }) => {
      const r = await agent.com.atproto.space.createRecord({
        repo,
        space,
        collection,
        rkey,
    ...
    args: ['repo', 'space', 'collection', 'record', 'rkey?'],
